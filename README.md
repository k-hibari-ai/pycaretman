...
- **モデルメタデータの確認**: `/saved_models_metadata` エンドポイントを利用すると、保存済みモデルのファイルサイズや更新日時をAPI経由で取得できます。
...
- **ローカルプレビュー**: `/preview_predictions/<ファイル名>` で保存済みの推論結果ファイルをプレビューし、行数・列数・主要な数値統計量を素早く確認できます。
...
学習アプリ (`app.py`) で保存されたモデルを推論アプリが参照できるよう、`inference_app.py` では以下の環境変数を使用できます。

| 変数名 | 説明 |
| --- | --- |
| `LEARNING_APP_RESULTS_FOLDER` | 学習アプリの保存モデルが配置されているディレクトリ。設定しない場合はリポジトリ直下の`models/`が自動的に利用されます。 |

パスが誤っている場合でも自動的に`models/`へフォールバックするため、ローカル環境でも安全に利用できます。
